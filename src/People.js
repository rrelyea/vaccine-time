...
function People() {
    ...
    function getStyle(user, schedule, dose) {
        ...
        var dosesNeeded = calculateDosesNeeded(schedule);
    ...
    function getDate(user, dose) {
        var dose = user.doses[dose-1];
        if (dose !== undefined) return dose.date;
    }

    function showAfter(user, dose) {
        if (user.doses.length < dose) {
            return user.schedule.doses[dose-1].after;
        }
    }
